...
pub const Dep = struct {
    ...
    pub fn clean_path(self: Dep) ![]const u8 {
        ...
        p = try std.mem.join(gpa, "/", &[_][]const u8{@tagName(self.type), p});
        return p;
    }

    pub fn clean_path_v(self: Dep) ![]const u8 {
        if (self.type == .http and self.version.len > 0) {
            return std.mem.join(gpa, "/", &[_][]const u8{"v", @tagName(self.type), self.version});
        }
        return std.mem.join(gpa, "/", &[_][]const u8{"v", try self.clean_path(), self.version});
    }
